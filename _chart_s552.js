...
[-12,0,12].forEach(off=>{let t=8*60+25+off;const tt=String(Math.floor(t/60)).padStart(2,'0')+':'+String(t%60).padStart(2,'0');
 const q=A.calculateBirthChart({...B,time:tt});const qp=Array.isArray(q.planets)?q.planets:Object.values(q.planets);
 console.log('SENS',String(off).padStart(3),tt,'lagna',q.lagna.signName,q.lagna.degreeInSign.toFixed(2),'| Moon',qp.find(p=>p.name==='Moon').signName);});
...
(ad.subPeriods||[]).forEach(p=>{if(new Date(p.endDate)>T)console.log('     PD',p.planet.padEnd(8),(''+p.startDate).slice(0,10),'->',(''+p.endDate).slice(0,10));});
d.mahaDashas.forEach(p=>{if(new Date(p.endDate)>new Date('2020-01-01')&&new Date(p.startDate)<new Date('2045-01-01'))console.log('MAHA',p.planet,(''+p.startDate).slice(0,10),'->',(''+p.endDate).slice(0,10));});
...
console.log('\nTRANSITS');
for(let y=2026;y<=2030;y++)for(const mo of [1,4,7,10]){const ds=y+'-'+String(mo).padStart(2,'0')+'-01';
