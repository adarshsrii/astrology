...
const LAT=28.2333,LON=83.6833,TZ='Asia/Kathmandu';
const res=A.calculateBirthChart({date:'1986-04-25',time:'04:00',latitude:LAT,longitude:LON,timezone:TZ});
...
[7,9].forEach(dv=>{const dc=A.calculateDivisionalChart(dv,plist,L.signNumber,L.degreeInSign);const lag=dc.lagnaSign.number;
...
const d=A.calculateVimshottariDasha(new Date('1986-04-24T22:15:00Z'),m.nakshatra,(m.longitude%13.3333333),3);
...
md.subPeriods.forEach(p=>{if(new Date(p.endDate)>new Date('2024-01-01'))console.log('  AD',p.planet,(''+p.startDate).slice(0,10),'→',(''+p.endDate).slice(0,10));});
