...
exports.calculateMonthlyPanchang = void 0;
const panchang_v2_1 = require("./panchang-v2");
/**
 ...
 */
function calculateMonthlyPanchang(year, month, latitude, longitude, timezone) {
    // Number of days in this month (day 0 of next month = last day of this month)
    const daysInMonth = new Date(year, month, 0).getDate();
    const days = [];
    let prevSunSign = null;
    for (let day = 1; day <= daysInMonth; day++) {
        const dateStr = `${year}-${String(month).padStart(2, '0')}-${String(day).padStart(2, '0')}`;
        const panchang = (0, panchang_v2_1.calculateFullPanchang)(dateStr, latitude, longitude, timezone);
        ...
        if (tithiNumber === 13)
            specialDays.push('Pradosh');
        if (tithiNumber === 4)
            specialDays.push('Chaturthi');
        if (tithiNumber === 14 && paksha === 'Krishna')
            specialDays.push('Shivaratri');
...
exports.calculateMonthlyPanchang = calculateMonthlyPanchang;
/**
