...
[-15,-8,0,8,15].forEach(off=>{let t=5*60+15+off;const tt=String(Math.floor(t/60)).padStart(2,'0')+':'+String(t%60).padStart(2,'0');
...
const dv9=A.calculateDivisionalChart(9,pl,L);console.log('\nD9 lagnaSign',dv9.lagnaSign);dv9.planets.forEach(p=>console.log('   ',p.planet.padEnd(8),p.vargaSignName));
...
if(nx){console.log('NEXT MD',nx.planet);nx.subPeriods.slice(0,6).forEach(p=>console.log('   AD',p.planet.padEnd(8),(''+p.startDate).slice(0,10),'->',(''+p.endDate).slice(0,10)));}
