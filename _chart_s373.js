...
const LAT=28.6139,LON=77.2090,TZ='Asia/Kolkata',DATE='1985-03-19',TIME='03:30';
[['Delhi',28.6139,77.2090],['Gurgaon',28.4595,77.0266],['Noida',28.5355,77.3910],['Ghaziabad',28.6692,77.4538]].forEach(([n,la,lo])=>{
 const r=A.calculateBirthChart({date:DATE,time:TIME,latitude:la,longitude:lo,timezone:TZ});
 console.log('NCR',n,'lagna',r.lagna.signName,r.lagna.degreeInSign.toFixed(2),r.lagna.nakshatra);});
...
if(nm){console.log('NEXT MD',nm.planet,(''+nm.startDate).slice(0,10),'→',(''+nm.endDate).slice(0,10));nm.subPeriods.slice(0,4).forEach(p=>console.log('  nAD',p.planet,(''+p.startDate).slice(0,10),'→',(''+p.endDate).slice(0,10)));}
['2026-07-29','2027-01-15','2027-07-15','2028-01-15'].forEach(dt=>{
