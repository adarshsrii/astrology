...
const PLANETARY_REMEDIES: Record<string, PlanetaryRemedy> = {
  Sun: {
    planet: 'Sun',
    gemstone: {
      name: 'Ruby',
 ...
 */
export const PLANET_DIRECTIONS: Record<string, string> = Object.fromEntries(
  Object.entries(PLANETARY_REMEDIES).map(([name, remedy]) => [name, remedy.direction]),
);

/** Direction for one graha, or null if the name is unknown. */
export function getPlanetDirection(planet: string): string | null {
  return PLANET_DIRECTIONS[planet] ?? null;
}
...
function isDebilitated(dignity: string): boolean {
  return dignity === 'debilitated' || dignity === 'peak_debilitated';
}

function isInDusthana(house: number): boolean {
  return DUSTHANA_HOUSES.includes(house);
}

function getAfflictionReason(planet: PlanetInput): string | null {
 ...
 */
export function getRemedies(planets: PlanetInput[]): RemedyResult {
  const weakPlanets: WeakPlanetRemedy[] = [];
 ...
 */
export function getPlanetRemedy(planet: string): PlanetaryRemedy | null {
