...
export interface MoonEvents { rise: Date | null; set: Date | null }
...
export function moonRiseSet(
  ...
  const dateOf = (jd: number) => new Date((jd - 2440587.5) * 86400000);

  /** Upper limb altitude in degrees: >= 0 means the moon is up. */
  const limbAltitude = (jd: number): number => {
    ...
    if (typeof ra !== 'number' || typeof dec !== 'number' || !distAU) return NaN;
    ...
    if (typeof gst !== 'number') return NaN;
    const hourAngle = ((gst * 15 + longitude) - ra) * DEG;
    const alt = Math.asin(
      Math.sin(latitude * DEG) * Math.sin(dec * DEG) +
      Math.cos(latitude * DEG) * Math.cos(dec * DEG) * Math.cos(hourAngle),
    ) / DEG;
  ...
  if (Number.isNaN(prev)) return { rise: null, set: null };

  for (let t = COARSE; t <= hours / 24; t += COARSE) {
    const jd = start + t;
    const cur = limbAltitude(jd);
    if (Number.isNaN(cur)) return { rise: null, set: null };
    ...
    if (crossedUp || crossedDown) {
      ...
      for (let m = 1; m <= 4; m++) {
        ...
        if (crossedUp ? v >= 0 : v <= 0) { hit = j; break; }
      }
      if (crossedUp && !rise) rise = dateOf(hit);
      if (crossedDown && !set) set = dateOf(hit);
      if (rise && set) break;
    }
    prevJd = jd; prev = cur;
  }
