...
// Adarsha Ghimire — 11 Sep 1996 AD, 22:30, Pokhara Nepal
const birth={date:'1996-09-11',time:'22:30',latitude:28.2096,longitude:83.9856,timezone:'Asia/Kathmandu'};
...
// 22:30 Kathmandu (+5:45) = 16:45 UTC
const d=A.calculateVimshottariDasha(new Date('1996-09-11T16:45:00Z'),m.nakshatra,(m.longitude%13.3333333),2);
...
console.log('--- Dasha 2025-2028 ---');
const W=new Date('2025-01-01'),X=new Date('2028-12-31');
d.mahaDashas.forEach(p=>{const s=new Date(p.startDate),e=new Date(p.endDate);if(e<W||s>X)return;console.log('MD',p.planet,(''+p.startDate).slice(0,10),'→',(''+p.endDate).slice(0,10));(p.subPeriods||[]).forEach(s2=>{const ss=new Date(s2.startDate),ee=new Date(s2.endDate);if(ee<W||ss>X)return;console.log('  AD',s2.planet,(''+s2.startDate).slice(0,10),'→',(''+s2.endDate).slice(0,10),'  ',s2.subPeriods?'':'');});});
fs.writeFileSync('/tmp/_chart_s102.json',JSON.stringify({lagnaSign:res.lagna.signName,lagnaSignNum:sn(res.lagna.signName),nakshatra:res.lagna.nakshatra,houses,planets,dasha:{current:d.currentDasha,timeline}}));
console.log('WROTE /tmp/_chart_s102.json');
