...
[-12,-6,0,6,12].forEach(off=>{let t=6*60+18+off;const tt=String(Math.floor(t/60)).padStart(2,'0')+':'+String(t%60).padStart(2,'0');
...
md.subPeriods.forEach(p=>{if(new Date(p.endDate)>new Date('2025-06-01')&&new Date(p.startDate)<new Date('2032-01-01'))console.log('   AD',p.planet.padEnd(8),(''+p.startDate).slice(0,10),'->',(''+p.endDate).slice(0,10));});
...
if(nx){console.log('NEXT MD',nx.planet,(''+nx.startDate).slice(0,10));(nx.subPeriods||[]).slice(0,4).forEach(p=>console.log('     AD',p.planet.padEnd(8),(''+p.startDate).slice(0,10),'->',(''+p.endDate).slice(0,10)));}
